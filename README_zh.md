...
### TTS 流式处理（重要）
- **不再预加载全部音频**，改为逐段后台预载
- `prepareNextAudio(index)`: 后台预载从当前位置开始的 3 段音频
- `processNextSequenceItem()`: 播放当前段时并行发送下一段 TTS 请求
- **表情与打字同步**：表情在第一个字出现时触发，而非先显示"思考中"
- **超时/失败处理**: `ttsFailedTexts` Set 记录失败的文本，之后直接显示文字不播放语音
- **文本长度限制**: 后端限制 500 字符，超过返回错误
- TTS 服务地址: `ws://127.0.0.1:9880`

### 对话序列分段机制
- 按**表情标签 `[xxx]`** 划分，每遇到一个新标签开始新的一段
- 有效标签: `祈祷`、`发光`、`翻花绳`、`好奇`、`泪`、`脸黑`、`脸红`、`生气`、`星星`
- 非有效标签会被当作普通文本显示
...
- **显存管理**：Agent 结束后自动卸载 embedding/reranker 模型，释放显存
